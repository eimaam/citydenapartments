...
export default function BookingsPage() {
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await bookingsApi.create({ roomId: form.roomId, guestName: form.guestName, guestPhone: form.guestPhone, guestEmail: form.guestEmail || undefined, numberOfGuests: Number(form.numberOfGuests) || 1, checkInDate: form.checkInDate, checkOutDate: form.checkOutDate, actualPricePerNight: price, discount: Number(form.discount) || 0, discountType: form.discountType, discountPercentage: form.discountType === 'percentage' ? Number(form.discountPercentage) || 0 : 0, totalAmountPaid: Number(form.totalAmountPaid), paymentMethod: form.paymentMethod, bookingSource: form.bookingSource });
      setShowCreate(false);
