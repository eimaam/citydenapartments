...
export default function CalendarPage() {
  ...
  const onCellClick = async (date: Date, room: any, booking?: any) => {
    if (booking) {
      ...
      setForm({
        ...
        actualPricePerNight: room.roomTypeId?.basePrice ?? 0, discount: 0, discountType: 'fixed', discountPercentage: 0,
  ...
  const recalcTotal = (price: number, discount: number, nights: number) => {
    updateField('totalAmountPaid', Math.max(0, (price * nights) - discount));
  };
  ...
  const onPriceChange = (price: number) => {
    updateField('actualPricePerNight', price);
    recalcTotal(price, form.discountType === 'percentage' ? form.discountPercentage : form.discount, form.useNights ? form.nights : computedNights);
