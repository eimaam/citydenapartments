...
import { Button, Input, Select, Option, Drawer, Badge, Table, UserRole, RoomStatus, type UserRoleType } from '@citydenapartments/shared';
import type { TableProps } from '@citydenapartments/shared';
import { Plus, Copy, Check, Search, Users } from 'lucide-react';
...
const MANAGER_ROLES: UserRoleType[] = [UserRole.Reception, UserRole.KitchenStaff, UserRole.SocialMediaManager, UserRole.HouseKeeper];
...
export default function StaffPage() {
  ...
  const [form, setForm] = useState({ email: '', name: '', role: UserRole.Reception as string });

  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      const u = await api.get<PaginatedData>(`/users?page=${page}&limit=${LIMIT}&search=${encodeURIComponent(search)}`);
  ...
  const openCreate = () => {
    ...
    setForm({ email: '', name: '', role: UserRole.Reception });
    setDrawer(true);
  };
  ...
  const save = async () => {
    ...
    try {
      if (edit) {
        await api.patch(`/users/${edit._id}`, { name: form.name });
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="flex items-center gap-3 mb-6"><span className="w-8 h-px bg-primary" /><span className="text-xs font-bold tracking-[0.15em] uppercase text-outline">Team</span></div>
      ...
      <Drawer open={drawer} onClose={() => setDrawer(false)} title={edit ? 'Edit Staff' : 'Add Staff'} size="sm"
        ...
        {createdAccount ? (
          ...
          <div className="space-y-4">
            ...
            {!edit && <Input size="lg" type="email" placeholder="Email address" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />}
