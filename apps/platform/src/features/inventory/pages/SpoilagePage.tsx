...
export default function SpoilagePage() {
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      toast('success', 'Spoilage approved. Stock deducted.');
      fetchAll();
  ...
  const handleReject = async (id: string) => {
    ...
    try {
      ...
      toast('success', 'Spoilage rejected.');
      fetchAll();
      if (detail?._id === id) setDetail(await inventoryApi.getSpoilage(id));
    } catch (e: any) { toast('error', e.message); }
    finally { setActionLoading(''); }
  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-serif text-2xl sm:text-3xl text-on-surface">Spoilage & Write-Offs</h1>
      </div>
      ...
      <Drawer open={!!detail} onClose={() => setDetail(null)} title="Spoilage Report Details" width={520}>
        {detail && (
