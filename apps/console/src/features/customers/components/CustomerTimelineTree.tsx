...
export default function CustomerTimelineTree({ customerId }: Props) {
  ...
  const fetchTimeline = useCallback(async () => {
    ...
    try {
      ...
      if (startDateStr) qs.set('startDate', startDateStr);

      const res = await api.get<CustomerTimelineResponse>(`/customers/${customerId}/timeline?${qs.toString()}`);
      setData(res);
    } catch (e: any) {
  ...
  const getNodeStyle = (eventType: CustomerTimelineEventType) => {
    switch (eventType) {
      case 'profile_created':
        return { bg: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/30', icon: Sparkles };
      case 'checked_in':
        return { bg: 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/30', icon: LogIn };
      case 'checked_out':
      ...
      case 'vip_discount_updated':
        return { bg: 'bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/30', icon: Percent };
      case 'laundry_bill':
        return { bg: 'bg-teal-500/10 text-teal-600 dark:text-teal-400 border-teal-500/30', icon: Shirt };
      default:
        return { bg: 'bg-slate-500/10 text-slate-600 border-slate-500/30', icon: Clock };
    }
  };
  ...
  if (loading && !data) {
    return (
      <div className="p-8 text-center space-y-3">
        ...
        <p className="text-xs text-outline font-medium">Loading Guest Ledger &amp; Timeline Tree...</p>
      </div>
    );
  }
