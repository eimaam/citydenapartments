...
<table class="receipt-table">
  ...
  <tbody>
    ${roomRowsHtml}
    ${booking.discount > 0 ? `<tr><td>Discount${booking.discountType === 'fixed' ? ' (Fixed)' : booking.discountPercentage ? ` (${booking.discountPercentage}%)` : ''}${booking.discountReason ? ` - ${booking.discountReason}` : ''}</td><td></td><td>-\u20A6${booking.discount.toLocaleString()}</td></tr><tr style="font-weight:600"><td>Net Room Subtotal</td><td></td><td>\u20A6${(subtotal - booking.discount).toLocaleString()}</td></tr>` : ''}
            ...
            {booking.discount > 0 && (
              <>
                <tr>
                  <td>
                    Discount
                    {booking.discountType === 'fixed' ? ' (Fixed)' : booking.discountPercentage ? ` (${booking.discountPercentage}%)` : ''}
                ...
                <tr style={{ fontWeight: 600, backgroundColor: '#f9fafb' }}>
                  <td>Net Room Subtotal</td>
                  <td></td>
                  <td>₦{(subtotal - booking.discount).toLocaleString()}</td>
                </tr>
              </>
            )}
